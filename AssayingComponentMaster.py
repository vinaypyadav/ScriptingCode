...
for _, row in assayingComponentMaster.iterrows():
    ...
    cursor.execute("""
        SELECT 1 FROM assaying_component_master
        WHERE LOWER(TRIM(assaying_parameter)) = %s AND is_deleted = false
    """, (normalized_name,))
    ...
    cursor.execute("""
        INSERT INTO assaying_component_master (
            id, assaying_parameter, description, is_active,
            created_by, updated_by, is_deleted, created_at, updated_at
        )
        VALUES (
            uuid_generate_v4(), %s, %s, true,
